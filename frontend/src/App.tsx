...
export default function App() {
  ...
  const [touched, setTouched] = useState<{ title: boolean; count: boolean }>({ title: false, count: false })

  const errors = useMemo(() => {
    const errs: { title?: string; count?: string } = {}
    ...
    return errs
  }, [jobTitle, count, touched])

  const canSubmit = jobTitle.trim().length >= 2 && typeof count === 'number' && count >= 1 && count <= 100 && !loading

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setTouched({ title: true, count: true })
    if (!canSubmit) return
